...
module.exports = function(path){
    ...
    (function handle(data){
        ...
        var aReg = /\.ani\-.*?\{.*?animation:\s?(.+?)\;.+?\}/g;
        var kReg = /@keyframes\s?(\D+?)\s?\{\s?(.+?)\}[\.\@]/g;
        var animations = [];
        var keys = [];
        var get = function(reg,n){
            var s = [];
            var r;
            while(!!(r = reg.exec(css))){
                s.push(r[n]);
            }
            return s;
        }
        animations = get(aReg,1);
        keys = get(kReg,2);

        for(var p=0;p<animations.length;p++){
            var animation = animations[p];
            var ani = animation.split(' ');
            var timer = {};

            var key = keys[p];
            var key2 = key.replace(/\s\s+/g,'').replace(/\}/g,'');
            var key3 = '';
            var percent = [];
